...
function edge_resp_and_custom_css() {
	...
	<?php  }
	if (!empty($edge_settings['edge_custom_css'] )  || ($edge_settings['edge_slider_content_bg_color']=='on' )){
		$edge_internal_css = '<!-- Custom CSS -->'."\n";
		$edge_internal_css .= '<style type="text/css" media="screen">'."\n";
		if (!empty($edge_settings['edge_custom_css']) ) {
			$edge_internal_css .= $edge_settings['edge_custom_css']."\n";
		}
...
function edge_customize_scripts() {
	...
	wp_enqueue_script( 'edge_customizer_custom', get_template_directory_uri() . '/inc/js/customizer-custom-scripts.js', array( 'jquery' ), '20140108', true );

	$edge_upgrade_links = array(
							'upgrade_link'              => esc_url('http://themefreesia.com/themes/edge'),
							'upgrade_text'              => __( 'Upgrade to Pro', 'edge' ),
							);
		wp_localize_script( 'edge_customizer_custom', 'edge_upgrade_links', $edge_upgrade_links );
		wp_enqueue_script( 'edge_customizer_custom' );
	wp_enqueue_style( 'edge_customizer_custom', get_template_directory_uri() . '/inc/js/edge-customizer.css');wp_enqueue_script( 'edge_customizer_custom' );
	}
			...
			$image_attributes = wp_get_attachment_image_src($attachment_id,'edge_slider_image');
				...
				$remove_link = $edge_settings['edge_slider_link'];
					...
					if ($excerpt != '') {
						...
						$edge_page_sliders_display .= '<h3>'.$excerpt.' </h3></div><!-- end .slider-text -->';
					}
